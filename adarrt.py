...
class AdaRRT():
    ...
    def build(self):
        ...
        for k in range(self.max_iter):
            # Goal biasing: occasionally sample near the goal to help tree grow towards it
            # This improves convergence rate
            if k % 10 == 0:  # Every 10th iteration, sample near goal
                # Sample uniformly in a sphere around the goal
                goal_direction = np.random.uniform(-1, 1, size=self.goal.state.shape)
                norm = np.linalg.norm(goal_direction)
                if norm > 1e-6:  # Avoid division by zero
                    goal_direction = goal_direction / norm
                    # Sample at random distance up to goal_precision * 2
                    distance = np.random.uniform(0, self.goal_precision * 2)
                    sample = self.goal.state + distance * goal_direction
                else:
                    # Fallback to uniform sampling if direction is zero
            ...
            nearest = self._get_nearest_neighbor(sample)
            
            # Attempt to extend towards sample
            new_node = self._extend_sample(sample, nearest)
            
            # Check for completion if we created a new node
            if new_node and self._check_for_completion(new_node):
                # Connect goal to the tree and trace path
    ...
    def _get_nearest_neighbor(self, sample):
        ...
        nearest_node = self.start  # Default to start node
        
        # Iterate through all nodes in the tree (excluding goal)
    ...
    def _extend_sample(self, sample, neighbor):
        ...
        """
        # Compute direction vector from neighbor to sample
        direction = sample - neighbor.state
        distance = np.linalg.norm(direction)
        
        # If sample is closer than step_size, use sample directly
        if distance <= self.step_size:
            new_state = sample
        else:
            # Normalize direction and scale by step_size
            direction_normalized = direction / distance
            new_state = neighbor.state + self.step_size * direction_normalized
        ...
        # Create new node and add as child
    ...
    def _trace_path_from_start(self, node=None):
        ...
        # Trace path backwards from node to start
        path = []
        current = node
        ...
        # Reverse to get path from start to node
        path.reverse()
        return path
    ...
    def _check_for_collision(self, sample):
        ...
        if self.ada_collision_constraint is None:
            return False
        # Try the constraint check - the API might work differently than expected
        # Some constraint APIs return False when satisfied (no collision)
        # Others return True when satisfied (no collision)
        # We'll try: return True if constraint is NOT satisfied (collision)
        constraint_result = self.ada_collision_constraint.is_satisfied(
            self.ada.get_arm_state_space(),
            self.ada.get_arm_skeleton(), sample)
        # If is_satisfied returns True when no collision, we negate to get collision status
        # If is_satisfied returns False when no collision, we use it directly
        # Try both: first try negation (most common)
